  ...
  return (
    <div className="preview-panel">
        ...
        <div className="preview-screen__content">
          {previewSlide ? (
            ...
            >
                  ...
                  <>
                    ...
                    <div
                      style={{
                        position: 'absolute',
                        inset: 0,
                        background: previewSlide.background.type === 'video' ? 'rgba(0,0,0,0)' : 'rgba(0,0,0,0.32)',
                        zIndex: 1,
                      }}
                    />
                  </>
              ...
              >
                  ...
                  >
                    <div
                      style={{
                        ...
                        textAlign: 'center',
                        lineHeight: '1.6',
                        fontSize: getPreviewTextSize(previewSlide, previewSlide.fontSize === 'large' ? 16 : previewSlide.fontSize === 'medium' ? 12 : 10, previewStageWidth),
                        fontWeight: 700,
                ...
                {previewSlide.type === 'video' && (
                  <div style={{ width: '100%', height: '100%', position: 'relative' }}>
                    ...
                    >
                      <button className="btn btn--ghost" style={{ padding: '2px 6px', fontSize: '10px' }} onClick={togglePauseResume}>
                        {previewVideoPaused ? 'Resume' : 'Pause'}
                      </button>
                      <button className="btn btn--ghost" style={{ padding: '2px 6px', fontSize: '10px' }} onClick={stopPlayback}>
                        Stop
                      </button>
                      <button className="btn btn--ghost" style={{ padding: '2px 6px', fontSize: '10px' }} onClick={toggleMute}>
                ...
                {previewSlide.type === 'pdf' && (
                  <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <div style={{ flex: 1, minHeight: 0, border: '1px solid rgba(255,255,255,0.15)', borderRadius: '4px', overflow: 'hidden' }}>
                      <PdfRenderer path={previewSlide.path} pageNumber={previewSlide.page || 1} fitMode={previewSlide?.fitMode || 'contain'} />
                    </div>
                    <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.7)', textAlign: 'left' }}>
                ...
                {previewSlide.type === 'bible' && (
                  <div style={{ width: '100%' }}>
                    <div
                      style={{
                        fontSize: getPreviewTextSize(previewSlide, previewSlide.fontSize === 'large' ? 14 : previewSlide.fontSize === 'medium' ? 11 : 9, previewStageWidth),
                        color: previewSlide.textColor || '#fff',
                        fontFamily: previewSlide.fontFamily || 'inherit',
                        whiteSpace: 'pre-line',
                        lineHeight: '1.8',
                    ...
                    <div style={{ marginTop: '10px', fontSize: '10px', color: 'rgba(255,255,255,0.72)', textAlign: 'right' }}>
                ...
                {previewSlide.type === 'lyrics' && (
                  <div style={{ width: '100%' }}>
                    {(previewSlide.songTitle || previewSlide.sectionTitle) && (
                      <div style={{ marginBottom: '8px', fontSize: '10px', color: 'rgba(255,255,255,0.75)' }}>
              ...
              {previewSplitEnabled && (
                <>
                  <div style={{ position: 'absolute', top: 0, bottom: 0, right: `${previewRightPanePercent}%`, width: '1px', background: 'rgba(255,255,255,0.2)', zIndex: 8 }} />
                  <div
                    style={{
                      position: 'absolute',
                      top: 0,
                  ...
                  >
                    {sceneConfig.enableCameraTestMode ? (
                      ...
                      >
                        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'repeating-linear-gradient(45deg, rgba(255,255,255,0.06) 0 12px, rgba(255,255,255,0.02) 12px 24px)' }} />
                        <div style={{ position: 'absolute', inset: '14px', border: '1px solid rgba(255,255,255,0.28)' }} />
                        <div style={{ position: 'absolute', left: 0, right: 0, top: '50%', height: '1px', background: 'rgba(255,255,255,0.32)' }} />
                        <div style={{ position: 'absolute', top: 0, bottom: 0, left: '50%', width: '1px', background: 'rgba(255,255,255,0.32)' }} />
                        <div style={{ position: 'absolute', left: '10px', top: '10px', fontSize: '10px', color: '#8ee7ff', fontWeight: 700, letterSpacing: '0.5px' }}>CAMERA</div>
                        <div style={{ position: 'absolute', right: '10px', bottom: '10px', fontSize: '10px', color: 'rgba(255,255,255,0.9)', fontFamily: 'monospace' }}>
                      ...
                      <video
                        ...
                        autoPlay
                        playsInline
                        style={{ width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${previewCameraScale})`, transformOrigin: 'center center' }}
                      />
                    )}
      ...
      <div className="preview-screen" style={{ aspectRatio: PREVIEW.ASPECT_RATIO_16_9 }}>
        ...
        <div className="preview-screen__content">
          {projectorQueue.length > 0 ? <span style={{ fontSize: '11px' }}>{nextQueueTitle}</span> : <span style={{ fontSize: '11px' }}>No content</span>}
        </div>
      </div>
      ...
      <div style={{ marginTop: 'auto' }}>
          ...
          <div className="cp-two-col-grid">
            <div>
              ...
              <input
                ...
                onChange={(e) => setTransitionDelayMs(Math.max(TRANSITION.MIN_MS, Math.min(TRANSITION.MAX_MS, Number(e.target.value || TRANSITION.MIN_MS))))}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setTransitionDurationMs(Math.max(TRANSITION.MIN_MS, Math.min(TRANSITION.MAX_MS, Number(e.target.value || TRANSITION.MIN_MS))))}
                className="cp-input-sm"
              />
            </div>
          </div>
        ...
        <div className="cp-panel-box" style={{ display: 'none' }}>
          <label className="cp-label-row" style={{ marginBottom: '8px' }}>
            <input type="checkbox" checked={sceneConfig.mode === 'split_camera'} onChange={(e) => setSceneConfig((prev) => ({ ...prev, mode: e.target.checked ? 'split_camera' : 'normal' }))} />
            Enable Camera Split (Left Content / Right Camera)
          </label>
          <div style={{ marginBottom: '8px' }}>
            <label className="cp-label-row" style={{ marginBottom: '8px' }}>
              <input type="checkbox" checked={sceneConfig.enableCameraTestMode === true} onChange={(e) => setSceneConfig((prev) => ({ ...prev, enableCameraTestMode: e.target.checked }))} />
          ...
          <div>
            <div className="cp-field-label">
              Right Camera Width (%): {Math.max(SCENE.CAMERA_PANE_MIN_PERCENT, Math.min(SCENE.CAMERA_PANE_MAX_PERCENT, sceneConfig.cameraPanePercent))}
            </div>
            <input
              type="range"
              ...
              step={1}
              value={Math.max(SCENE.CAMERA_PANE_MIN_PERCENT, Math.min(SCENE.CAMERA_PANE_MAX_PERCENT, sceneConfig.cameraPanePercent))}
          ...
          <div style={{ marginTop: '8px' }}>
            <div className="cp-field-label">
              Camera Center Crop (%): {Math.max(SCENE.CAMERA_CROP_MIN_PERCENT, Math.min(SCENE.CAMERA_CROP_MAX_PERCENT, sceneConfig.cameraCenterCropPercent || SCENE.CAMERA_CROP_DEFAULT_PERCENT))}
            </div>
            <input
              type="range"
              ...
              step={5}
              value={Math.max(SCENE.CAMERA_CROP_MIN_PERCENT, Math.min(SCENE.CAMERA_CROP_MAX_PERCENT, sceneConfig.cameraCenterCropPercent || SCENE.CAMERA_CROP_DEFAULT_PERCENT))}
        ...
        <div className="cp-meta-list">
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
            <button className="btn btn--ghost" style={{ flex: 1, padding: '6px 8px', fontSize: '11px' }} onClick={handleExportSetupBundle} disabled={setupTransferBusy}>
              Export Setup
            </button>
            <button className="btn btn--ghost" style={{ flex: 1, padding: '6px 8px', fontSize: '11px' }} onClick={handleImportSetupBundle} disabled={setupTransferBusy}>
              Import Setup
            </button>
          </div>
          {setupTransferBusy && <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>Processing setup package...</div>}
        </div>
      </div>
    </div>
  );
