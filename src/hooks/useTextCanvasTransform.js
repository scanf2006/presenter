...
export default function useTextCanvasTransform({
  ...
  }, [textCanvasRef, textLayout, textSizePx]);
  ...
  useEffect(() => {
    const onMouseMove = (event) => {
      ...
      if (state.mode === 'drag') {
        ...
        const halfWidthPct = layerRect?.width ? (layerRect.width / rect.width) * TEXT_CANVAS.DRAG_BOUND_CENTER_PERCENT : TEXT_EDITOR.LAYOUT_X_MIN;
        const halfHeightPct = layerRect?.height ? (layerRect.height / rect.height) * TEXT_CANVAS.DRAG_BOUND_CENTER_PERCENT : TEXT_EDITOR.LAYOUT_Y_MIN;
        const minX = Math.max(TEXT_CANVAS.DRAG_BOUND_MIN_PERCENT, Math.min(TEXT_CANVAS.DRAG_BOUND_CENTER_PERCENT, halfWidthPct));
        const maxX = Math.min(TEXT_CANVAS.DRAG_BOUND_MAX_PERCENT, Math.max(TEXT_CANVAS.DRAG_BOUND_CENTER_PERCENT, 100 - halfWidthPct));
        const minY = Math.max(TEXT_CANVAS.DRAG_BOUND_MIN_PERCENT, Math.min(TEXT_CANVAS.DRAG_BOUND_CENTER_PERCENT, halfHeightPct));
        const maxY = Math.min(TEXT_CANVAS.DRAG_BOUND_MAX_PERCENT, Math.max(TEXT_CANVAS.DRAG_BOUND_CENTER_PERCENT, 100 - halfHeightPct));
        ...
        if (Math.abs(nextX - TEXT_CANVAS.SNAP_CENTER_PERCENT) <= snapThreshold && TEXT_CANVAS.SNAP_CENTER_PERCENT >= minX && TEXT_CANVAS.SNAP_CENTER_PERCENT <= maxX) {
          nextX = TEXT_CANVAS.SNAP_CENTER_PERCENT;
          guide.vertical = true;
        }
        if (Math.abs(nextY - TEXT_CANVAS.SNAP_CENTER_PERCENT) <= snapThreshold && TEXT_CANVAS.SNAP_CENTER_PERCENT >= minY && TEXT_CANVAS.SNAP_CENTER_PERCENT <= maxY) {
      ...
      if (state.mode === 'resize') {
        ...
        const baseSize = Number(state.startTextSizePx || textSizePx || TEXT_EDITOR.DEFAULT_SIZE_PX) * Number(state.startLayout?.scale || TEXT_EDITOR.LAYOUT_DEFAULT.scale);
        const nextSizePx = Math.round(clamp(baseSize + (delta / TEXT_CANVAS.RESIZE_DIVISOR), TEXT_EDITOR.SIZE_INPUT_MIN_PX, TEXT_EDITOR.SIZE_CLAMP_MAX_PX));
